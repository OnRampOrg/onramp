...
function Workspace(data){
	...
	self.refreshJobs = function () {
		...
		$.getJSON( "http://flux.cs.uwlax.edu/onramp/api/workspaces/" + self.id() + "/jobs?apikey=" + JSON.parse(self.auth_data).apikey,
			//self.auth_data,
			function (data){
				// {"status": 0,
	...
	self.refreshPCEModPairs = function () {
		...
		$.getJSON( "http://flux.cs.uwlax.edu/onramp/api/workspaces/" + self.id() + "/pcemodulepairs?apikey=" + JSON.parse(self.auth_data).apikey,
			//self.auth_data,
			function (data){
				// {"status": 0,
				//  "status_message": "Success",
	...
	self.refreshUsers = function () {
		...
		$.getJSON( "http://flux.cs.uwlax.edu/onramp/api/workspaces/" + self.id() + "/users?apikey=" + JSON.parse(self.auth_data).apikey,
			//self.auth_data,
			function (data){
				// {"status": 0,
				//  "status_message": "Success",
	...
	self.addPCEModPair = function () {
		...
		$.ajax({
	      type: 'POST',
	      url: 'http://flux.cs.uwlax.edu/onramp/api/admin/workspace/' + self.id() + '/pcemodulepairs/' + self.new_pce_id() + '/' + self.new_mod_id() + '?apikey=' + JSON.parse(this.auth_data).apikey,
	...
	self.createWorkspace = function () {
		$.ajax({
	      type: 'POST',
	      url: 'http://flux.cs.uwlax.edu/onramp/api/admin/workspace?apikey=' + JSON.parse(self.auth_data).apikey,
	      //data: JSON.stringify({'password':this.password(), 'username':this.username(), 'is_admin':this.isAdmin(), 'is_enabled':this.isEnabled(), 'email':this.email(), 'full_name':this.fullName()}),
