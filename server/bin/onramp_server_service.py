#!src/env/bin/python
"""Control the OnRamp Server REST server.

Usage: ./bin/onramp_server_service.py COMMAND [COMMAND_ARGS]

Commands:
    start
        Starts the OnRamp Server.

    stop
        Stops the OnRamp Server.

    restart
        Restarts the OnRamp Server.
"""
import os
import shutil
import sys
...
_pidfile = '.onrampServer.pid'
_src_dir = 'src'
_script_name = 'OnRampServer.py'

def _getPID():
    """Get PID from specified PIDFile.
    
    Returns:
        int: >0 -- OnRamp Server PID
...
def _status():
    """Determine status of the OnRamp Server"""
