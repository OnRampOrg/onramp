...
void shuffle_deck(int deck[], int num_cards) {
    ...
    int mid = rand_int_between(low, hi);
...
int pick_card (int deck[], int *num_cards){
    int loc = rand_int_between(0, (*num_cards) - 1);
    int card = deck[loc];
    int i;
...
int test_one_hand(){
    ...
    init_deck(deck); //create & shuffle a new deck
	
    draw_hand(deck, hand); //go pick cards from deck
